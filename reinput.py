    ...
    try:
        ...
        if len(datas) > 1:
            ...
            for data in datas:
                ...
                cursor.execute("SELECT * FROM changes WHERE href LIKE %s AND title LIKE %s AND likes LIKE %s AND stock LIKE %s AND price LIKE %s",(link,title,likes,stock,prices,time))
                datos = cursor.fetchall()
                if len(datos) >= 1:
                    print("base",title,likes,stock,prices,time)
                    print("checking")
                    break
                else:
